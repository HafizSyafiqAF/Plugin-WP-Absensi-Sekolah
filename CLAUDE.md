...
**Versi skema + migration runner (ADA):** `Installer::DB_VERSION` (`2.4.0`) + option `absensi_db_version`. `maybe_upgrade()` jalan tiap `plugins_loaded`: bila versi tersimpan < `DB_VERSION` → jalankan migrasi per-versi + `create_tables()` (dbDelta) + re-seed options. **Tak perlu deactivate/activate** untuk sinkron skema. Migrasi breaking (rename tabel/kolom / ubah tipe kolom) yang dbDelta tak bisa: `migrate_to_v2()` (RENAME TABLE + ALTER, idempotent) untuk <2.0.0, `migrate_to_v2_1()` (ALTER MODIFY `group.tipe` ENUM→VARCHAR, guard `column_is_enum`) untuk <2.1.0 — dipanggil SEBELUM `create_tables()`. Idempotent via cek `table_exists`/`column_exists`/`column_is_enum`/`index_exists`. `bersihkan_yatim()` (<2.3.0, SETELAH `create_tables()` karena butuh tabelnya ada) menyapu rekap tanpa user & jadwal tanpa group yang terlanjur menumpuk sebelum cascade ada. `bersihkan_role_warisan()` (<2.4.0) membuang role/cap pra-pivot (`absensi_admin`/`absensi_siswa`/`orang_tua` + 5 cap absen di administrator; `guru`+`absensi_rfid` dipertahankan), memindah user yang cuma pegang role warisan ke `subscriber`.

**Cascade hapus (v2.3.0, tak ada FK di skema — cascade dikerjakan endpoint):** `DELETE /users/{id}` & `POST /users/bulk-delete` ikut menghapus **rekap** user itu (respons + `rekap_dihapus`); `DELETE /group/{id}` ikut menghapus **jadwal** group itu (respons + `jadwal_dihapus`; guard 409 `group_ada_user` tetap, dan saat ditolak jadwal tak tersentuh). Alasan bukan sekadar kerapian: `/laporan/summary` menghitung `COUNT(*)` dari rekap **tanpa JOIN users** → baris yatim menaikkan angka Hadir/Telat; di list/export (LEFT JOIN) ia muncul tanpa nama; dan karena `rekap.user_id` cuma angka, **id yang dipakai ulang** (MariaDB/MySQL 5.7 me-reset `AUTO_INCREMENT` ke `MAX(id)+1` tiap restart) membuat riwayat orang lama nempel ke user baru. Konsekuensi yang disengaja: **hapus user = riwayat absensinya hilang dari laporan lama** (modal FE menyebutkannya). Kalau kelak butuh riwayat tetap utuh → arsip/soft-delete, bukan mengembalikan baris yatim.
...
- **Role `guru` AKTIF kembali** (fitur Akun Guru): di-seed `Installer::seed_roles()` (caps `read` + `absensi_rfid`, tanpa cap admin lain). Cap `absensi_rfid` juga di administrator. Gate/blok terkait di §Akun Guru. (Role pra-pivot lain — `orang_tua`/`absensi_admin`/`absensi_siswa` + 5 cap absen di administrator — **dibersihkan migrasi v2.4.0** `bersihkan_role_warisan()`; tak lagi nyangkut. User yang cuma pegang role warisan dipindah ke `subscriber` agar tak role-less.)
...
- **Identitas + Keluar di kiosk** ([public/views/guru.php](public/views/guru.php)): chip "Login sebagai: [Nama]" + tombol Keluar (`wp_logout_url(get_permalink())` → logout balik ke kiosk → gate mengalihkan ke wp-login). Render server-side (`wp_get_current_user()`, page login-gated). Login **persisten** (tak ada auto-logout — kiosk dipakai sebentar saat jam masuk). Chip = kejelasan operator; identitas absen tetap dari `rfid_uid` siswa.
...
- ~~Hapus user/group meninggalkan baris yatim~~ **SUDAH DITANGANI (v2.3.0)** — cascade di endpoint + `bersihkan_yatim()` untuk sisa lama. Lihat §Database.
...
- **Role `guru` AKTIF** (fitur Akun Guru): di-seed `seed_roles()`, dibaca gate `absensi_rfid` (§Akun Guru). Role pra-pivot LAIN (`absensi_admin`/`absensi_siswa`/`orang_tua`) + 5 cap absen di administrator **SUDAH DIBERSIHKAN (v2.4.0)** — `bersihkan_role_warisan()` di `maybe_upgrade()` membuangnya tiap upgrade (tak nunggu DELETE plugin); user yang cuma pegangnya dipindah ke `subscriber`. Daftar cap/role WAJIB sinkron dengan `uninstall.php`.
